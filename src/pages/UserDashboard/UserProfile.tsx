...
const UserProfile: React.FC = () => {
    ...
    const tabs = [
        ...
        { id: 'coupons' as ProfileTabType, label: t('profile.coupons') || 'Coupons', icon: Ticket }, // Fallback if key missing
    ];
            ...
            <div className="max-w-4xl mx-auto space-y-6 md:space-y-8">
                ...
                <div className={`rounded-xl p-6 md:p-8 shadow-lg text-white mb-6 md:mb-8 transition-all ${currentSubscription
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                            <h3 className="text-lg md:text-xl font-bold mb-2">{t('profile.currentSubscription')}</h3>
                            <p className="text-white/90 mb-4 text-xs font-semibold uppercase tracking-wider">
                                {currentSubscription?.plan?.name || currentSubscription?.planName || t('profile.noActivePlan')}
                            </p>
                            ...
                            {currentSubscription && (
                                <p className="text-sm text-white/80 mt-2">
                                    {['active', 'trialing', 'succeeded', 'year'].includes(currentSubscription?.status?.toLowerCase() || '') ? t('profile.renewsOn') : t('profile.expiredOn')} {new Date(currentSubscription.renewalDate || currentSubscription.endDate || Date.now()).toLocaleDateString()}
                                </p>
                            )}
                        </div>
                        <Button
                        ...
                        >
                            {currentSubscription ? t('profile.managePlan') : t('profile.upgradeToPro')}
                        </Button>
                    </div>
                </div>
