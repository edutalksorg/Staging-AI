...
const ResetPasswordPage: React.FC = () => {
  ...
  const onSubmit = async (data: FormData) => {
    if (!userId || !token) {
      dispatch(showToast({ message: t('auth.invalidResetLinkParams'), type: 'error' }));
      return;
    }
    ...
    try {
      ...
      dispatch(showToast({ message: t('auth.passwordResetSuccess'), type: 'success' }));
      navigate('/login');
    } catch (err: any) {
      const msg = err?.response?.data?.message || t('auth.failedToResetPassword');
  ...
  return (
    <div className="min-h-dvh bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="card">
          <h1 className="text-2xl font-bold text-center mb-4 text-slate-900 dark:text-white">{t('auth.resetPasswordTitle')}</h1>

          {!userId || !token ? (
            <div className="p-4 text-center">
              <p className="text-sm text-slate-600 dark:text-slate-400">{t('auth.invalidResetLink')}</p>
              <p className="mt-4">
                <Link to="/forgot-password" className="text-primary-600 hover:underline">{t('auth.requestNewLink')}</Link>
              </p>
            </div>
          ) : (
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-900 dark:text-white mb-2">{t('auth.newPassword')}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-900 dark:text-white mb-2">{t('auth.confirmPassword')}</label>
              ...
              <Button type="submit" variant="primary" fullWidth isLoading={isLoading}>{t('auth.setNewPassword')}</Button>
            </form>
          )}
        </div>
      </div>
